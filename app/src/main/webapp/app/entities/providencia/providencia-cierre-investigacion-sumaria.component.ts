...
@Component({
    selector: 'jhi-providencia-cierre-investigacion-sumaria',
    templateUrl: './providencia-cierre-investigacion-sumaria.component.html'
})
export class ProvidenciaCierreInvestigacionSumariaComponent implements OnInit {
...
export class ProvidenciaCierreInvestigacionSumariaPopupComponent implements OnInit, OnDestroy {
    ...
    ngOnInit() {
        this.activatedRoute.data.subscribe(({ providencia }) => {
            setTimeout(() => {
                this.ngbModalRef = this.modalService.open(ProvidenciaCierreInvestigacionSumariaComponent as Component, {
                    size: 'lg',
                    backdrop: 'static'
                });
