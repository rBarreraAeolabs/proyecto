...
@Component({
    selector: 'jhi-providencia-memo-conductor',
    templateUrl: './providencia-memo-conductor.component.html'
})
export class ProvidenciaMemoConductorComponent implements OnInit {
    ...
    memoConductor(id: number) {
        ...
        this.providenciaService.memoConductor(this.providenciaResponse).subscribe(res => {
            this.eventManager.broadcast({
                name: 'providencia',
                content: 'Providencia memo conductor'
            });
...
@Component({
    selector: 'jhi-providencia-memo-conductor-popup',
    template: ''
})
export class ProvidenciaMemoConductorPopupComponent implements OnInit, OnDestroy {
    ...
    ngOnInit() {
        this.activatedRoute.data.subscribe(({ providencia }) => {
            setTimeout(() => {
                this.ngbModalRef = this.modalService.open(ProvidenciaMemoConductorComponent as Component, {
                    size: 'lg',
                    backdrop: 'static'
                });
