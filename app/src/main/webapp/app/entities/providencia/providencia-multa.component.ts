...
@Component({
    selector: 'jhi-providencia-multa',
    templateUrl: './providencia-multa.component.html'
})
export class ProvidenciaMultaComponent implements OnInit {
...
export class ProvidenciaMultaPopupComponent implements OnInit, OnDestroy {
    ...
    ngOnInit() {
        this.activatedRoute.data.subscribe(({ providencia }) => {
            setTimeout(() => {
                this.ngbModalRef = this.modalService.open(ProvidenciaMultaComponent as Component, {
                    size: 'lg',
                    backdrop: 'static'
                });
