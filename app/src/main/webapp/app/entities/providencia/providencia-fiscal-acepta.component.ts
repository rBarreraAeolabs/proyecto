...
@Component({
    selector: 'jhi-providencia-fiscal-acepta',
    templateUrl: './providencia-fiscal-acepta.component.html'
})
export class ProvidenciaFiscalAceptaComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        if ( this.providencia.requisito === 'FISCAL_NOTIFICADO') {
            console.log('el usuario es fiscal acepta providencia');
            this.isAceptar = true;
        }
    }
    get providencias() {
    ...
    fiscalAcepta(id: number ) {
        ...
        if (this.providencia.requisito === 'FISCAL_NOTIFICADO') {
            this.providenciaService.aceptar(this.providenciaResponse).subscribe(res => {
...
@Component({
    selector: 'jhi-providencia-fiscal-acepta-popup',
    template: ''
})
export class ProvidenciaFiscalAceptaPopupComponent implements OnInit, OnDestroy {
    ...
    ngOnInit() {
        this.activatedRoute.data.subscribe(({ providencia }) => {
            setTimeout(() => {
                this.ngbModalRef = this.modalService.open(ProvidenciaFiscalAceptaComponent as Component, {
                    size: 'lg',
                    backdrop: 'static'
                });
